...
it('keeps demo fixtures out of public navigation and normalizes generated labels', function (): void {
    $navigationDemoCreator = resolve(NavigationDemoCreator::class);
    ...
    $home = Page::factory()
        ->site($site)
        ->type($homePageType)
        ->withTranslations(capell_test_collect([$language]), ['title' => 'Home', 'label' => 'Home'])
        ->create(['name' => 'Home', 'visible_from' => now()->subDay()]);
    $publicPage = Page::factory()
        ->site($site)
        ->type($defaultPageType)
        ->withTranslations(capell_test_collect([$language]), [
            'title' => 'A practical brief for a shared workshop | Field Office',
            'label' => null,
        ])
        ->create(['name' => 'Workshop Brief', 'visible_from' => now()->subDay()]);
    $themeDemoPage = Page::factory()
        ->site($site)
        ->type($defaultPageType)
        ->withTranslations(capell_test_collect([$language]), ['title' => 'Search Empty State', 'label' => 'Search Empty State'])
        ->create([
            'name' => 'Search Empty State',
            'meta' => ['theme_demo' => ['theme_key' => 'fixture', 'surface' => 'search-empty']],
            'visible_from' => now()->subDay(),
        ]);
    $kitchenSinkPage = Page::factory()
        ->site($site)
        ->type($defaultPageType)
        ->withTranslations(capell_test_collect([$language]), ['title' => 'Kitchen Sink Demo Page', 'label' => 'Kitchen Sink Demo Page'])
        ->create([
            'name' => 'Kitchen Sink Demo Page',
            'meta' => ['demo_fixture' => 'kitchen-sink'],
            'visible_from' => now()->subDay(),
        ]);
    $footerExcludedPage = Page::factory()
        ->site($site)
        ->type($defaultPageType)
        ->withTranslations(capell_test_collect([$language]), [
            'title' => 'Footer Fixture',
            'label' => 'Footer Fixture',
            'meta' => ['exclude_from_footer' => true],
        ])
        ->create(['name' => 'Footer Fixture', 'visible_from' => now()->subDay()]);

    $stalePageItem = static fn (Page $page): array => [
        'label' => $page->name,
        'type' => NavigationItemType::Page->value,
        'data' => [
            'site_id' => $site->id,
    ...
    Navigation::factory()
        ...
        ->create([
            ...
            'blueprint_id' => $navigationType->id,
            'items' => [
                'stale-theme-demo' => $stalePageItem($themeDemoPage),
                'stale-kitchen-sink' => $stalePageItem($kitchenSinkPage),
            ],
        ]);
    Navigation::factory()
        ->site($site)
        ...
        ->create([
            ...
            'items' => [
                'stale-theme-demo' => $stalePageItem($themeDemoPage),
                'stale-kitchen-sink' => $stalePageItem($kitchenSinkPage),
                'stale-footer-fixture' => $stalePageItem($footerExcludedPage),
            ],
        ]);
    ...
    $footer = Navigation::query()
        ...
        ->where('key', NavigationHandle::Footer->value)
        ->firstOrFail();
    $mainItems = navigationDemoCreatorItems($main);
    $footerItems = navigationDemoCreatorItems($footer);

    expect($mainItems->pluck('label')->all())
        ->toContain('A practical brief for a shared workshop')
        ->not()->toContain($themeDemoPage->name, $kitchenSinkPage->name)
        ->and($footerItems->pluck('label')->all())
        ->toContain('A practical brief for a shared workshop')
        ->not()->toContain($themeDemoPage->name, $kitchenSinkPage->name, $footerExcludedPage->name);
});
