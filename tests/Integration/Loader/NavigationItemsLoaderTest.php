...
it('loads page navigation when the current page has no URL row', function (): void {
    ...
    $currentPage = Page::factory()->site($site)->withTranslations(slug: '/')->create();
    $currentPage->pageUrls()->delete();
    $currentPage->unsetRelation('pageUrl');
    ...
    $items = (new NavigationItemsLoader(
    ...
    expect($items)->toHaveCount(1)
        ->and(navigationLoaderItem($items, 0)->data['url'])->toBe('#')
        ->and(navigationLoaderItem($items, 0)->active)->toBeTrue();
});
