...
/**
 * Base class shared by all SaaS theme widgets.
 *
 ...
 */
abstract class AbstractSaasWidget
{
    public string $name;
    ...
    {
        $title = (string) ($data['title'] ?? $data['heading'] ?? $this->name);

        return '<section data-widget="' . e(static::class) . '"><h2>' . e($title) . '</h2></section>';
    }
}

if (! function_exists('Capell\\Themes\\Saas\\Widgets\\e')) {
    /**
