...
class LayoutBuilder extends Component implements HasActions, HasForms
{
    ...
    private function resolveEditableWidgetAsset(array $arguments): WidgetAsset
    {
        ...
        if (! $asset) {
            throw new MissingWidgetAssetException($widget, $type, $index, $arguments);
        }
    ...
    {
        // reset any queued deletions when starting fresh
        $this->pendingAssetDeletes = [];
    ...
    private function setupWidgetAssets(string $containerKey, int $widgetIndex, array $widgetAssets, ?Collection $allWidgetAssets): Collection
    {
        ...
        if (! $allWidgetAssets || $allWidgetAssets->isEmpty()) {
            return $assets;
        }
        ...
        $existingAssets = $existingIds === []
            ...
            : $model::query()
                ...
                ->when(
                    ...
                    fn (BuilderContract $query) => $query->orderByRaw('CASE id ' . implode(' ', array_map(fn ($id) => "WHEN {$id} THEN " . array_search($id, $existingIds), $existingIds)) . ' END'),
