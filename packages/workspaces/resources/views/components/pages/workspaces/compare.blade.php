...
<x-filament-panels::page>
    ...
    @if($checkResults)
        <div class="mt-6">
            <h3 class="text-lg font-semibold mb-3">{{ __('capell-admin::workspace.compare.checks_heading') }}</h3>
            <div class="space-y-3">
                @foreach($checkResults as $checkResult)
                    ...
                    @endphp
                    <div class="rounded-lg border p-4 {{ $checkResult->isClean() ? 'border-success-200 bg-success-50' : 'border-' . $severityColor . '-200 bg-' . $severityColor . '-50' }}">
                        <div class="flex items-center gap-2">
                            <span class="font-medium">{{ $checkResult->label }}</span>
                            <x-filament::badge :color="$severityColor">
                                {{ $checkResult->severity->value }}
                            </x-filament::badge>
                            @if($checkResult->isClean())
                                <x-filament::badge color="success">{{ __('capell-admin::workspace.compare.check_passed') }}</x-filament::badge>
                            @endif
                        </div>
                        @if(! $checkResult->isClean())
                            ...
                            @if($checkResult->entityRefs)
                                <p class="mt-1 text-sm text-gray-500">{{ trans_choice('capell-admin::workspace.compare.check_entity_refs', count($checkResult->entityRefs)) }}</p>
                            @endif
                        @endif
                    </div>
                @endforeach
