...
namespace Capell\ContentBlocks\Filament\Concerns;
...
trait HasAssetsRelationManager
{
    protected static function createResourcesAction(): Action
    {
        return CreateAction::make()
            ->label(__('capell-content-blocks::button.add_asset'))
            ->color('primary')
            ->successNotificationTitle(__('capell-content-blocks::message.asset_added'))
                ...
                function (Select $select) use ($asset): Select {
                    ...
                    return $select->createOptionForm(
                        ...
                        ->createOptionUsing(function (Select $component, array $data) use ($asset, $adminAsset, $createOptionUsing): int|string {
                            ...
                            Notification::make()
                                ->title(__('capell-content-blocks::message.asset_created_successfully', ['name' => $asset->name]))
                                ->body($page->name)
                                ->send();
